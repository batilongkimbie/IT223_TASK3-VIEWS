...
--
-- Stand-in structure for view `hr_view`
-- (See below for the actual view)
--
CREATE TABLE `hr_view` (
`Employee ID` int(11) unsigned
,`Name` varchar(46)
,`job_title` varchar(35)
,`Employment Date` varchar(23)
,`Manager` varchar(46)
,`department_name` varchar(30)
,`Location` varchar(182)
);
...
--
-- Structure for view `hr_view`
--
DROP TABLE IF EXISTS `hr_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `hr_view`  AS SELECT `e`.`employee_id` AS `Employee ID`, concat(`e`.`first_name`,' ',`e`.`last_name`) AS `Name`, `j`.`job_title` AS `job_title`, concat(`jh`.`start_date`,' - ',`jh`.`end_date`) AS `Employment Date`, concat(`e1`.`first_name`,' ',`e1`.`last_name`) AS `Manager`, `d`.`department_name` AS `department_name`, concat(`l`.`street_address`,', ',`l`.`postal_code`,', ',`l`.`city`,', ',`l`.`state_province`,', ',`c`.`country_name`,', ',`r`.`region_name`) AS `Location` FROM (((((((`employees` `e` join `jobs` `j` on(`e`.`job_id` = `j`.`job_id`)) left join `job_history` `jh` on(`e`.`employee_id` = `jh`.`employee_id`)) left join `employees` `e1` on(`e`.`manager_id` = `e1`.`employee_id`)) left join `departments` `d` on(`e`.`department_id` = `d`.`department_id`)) left join `locations` `l` on(`d`.`location_id` = `l`.`location_id`)) left join `countries` `c` on(`l`.`country_id` = `c`.`country_id`)) left join `regions` `r` on(`c`.`region_id` = `r`.`region_id`)) ORDER BY `e`.`employee_id` ASC ;
